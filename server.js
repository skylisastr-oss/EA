...
.catch(err => {
  ...
  console.log('4. Click "Add Current IP Address" or use 0.0.0.0/0 for dev');
...
app.get('/api/attendance/today', async (req, res) => {
  try {
    ...
    if (req.query.format === 'csv') {
      const csv = [
        'Student ID,Name,Course,Check-in Time,Confidence',
        ...attendanceRecords.map(a => 
          `${a.studentId},${a.name},${a.course},${new Date(a.checkInTime).toLocaleString()},${a.confidence || 'N/A'}`
        )
      ].join('\n');

      res.setHeader('Content-Type', 'text/csv');
      res.setHeader('Content-Disposition', `attachment; filename=attendance_${today.replace(/\//g, '-')}.csv`);
      return res.send(csv);
    }
