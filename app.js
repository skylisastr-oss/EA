  ...
  try {
    ...
    Swal.fire({
      ...
      html: `
        ...
        1. Backend server is deployed on Render<br>
        2. Server is running properly<br>
        3. MongoDB is connected<br>
        <br>
        <strong>Expected API:</strong> ${API}
      `,
      icon: "error",
...
document.getElementById("registerStudent").addEventListener("click", async () => {
  ...
  try {
    ...
    Swal.fire("Error", "Could not connect to server. Check server status on Render.", "error");
  }
});
...
async function loadStudents() {
  try {
    ...
    document.getElementById("studentsList").innerHTML = 
      `<p style="color: red;">❌ Could not load students: ${error.message}</p>`;
  }
}
...
async function loadAttendance() {
  try {
    ...
    document.getElementById("attendanceTable").innerHTML = 
      `<p style="color: red;">❌ Could not load attendance: ${error.message}</p>`;
  }
}
